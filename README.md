...
잦은 디자인 변경과 피드백 반영 과정에서 **작업 내역 유실을 방지**하고 **안전한 롤백**을 보장하기 위해 다음 원칙을 준수합니다.

1. **문서화 우선 (Documentation First)**
   - 모든 주요 변경사항, 업주 피드백, PMS 연동 스펙은 코드를 수정하기 전에 `docs/` 폴더 내의 Markdown(`.md`) 파일로 먼저 기록합니다.
2. **CHANGELOG 유지**
   - `CHANGELOG.md` 파일에 날짜별, 버전별 변경 내역을 상세히 기록합니다.
3. **작업 지시서 기반 개발**
   - 새로운 기능 추가나 대규모 디자인 변경 시 `docs/templates/task_template.md`를 복사하여 구체적인 작업 지시서를 작성한 후 작업을 진행합니다.
4. **강제 푸시(Force Push) 금지**
   - 히스토리 유실의 주원인인 `git push -f` 명령어 사용을 엄격히 금지합니다.
   - 이전 상태로 돌아가야 할 경우 `git revert`를 사용하거나 새로운 커밋으로 덮어쓰는 방식을 채택합니다.
